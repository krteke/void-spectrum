...
拥有平台无关的值类型和完全解析的强类型主题表示。可以暴露可选的序列化支持，但不能解析主题文件。

### `spectrum-schema`

拥有部分配置结构、令牌引用、主题元数据以及格式特定的加载功能。Schema 值可以是不完整的，因为 resolver 会提供默认值和派生值。
...
拥有优先级、合并、引用展开、循环检测、类型检查和契约验证。其输出是一个完整的 `spectrum-core` 主题。

### `spectrum-codegen`

拥有类型化令牌契约的编译期 Rust 源码生成能力。它在构建脚本中解析和解析静态主题文件，生成普通 Rust 源文件供消费 crate 从 `OUT_DIR` include，使 IDE 诊断和补全可以感知生成的令牌字段。
